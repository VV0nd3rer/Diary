...
$.get("get-sights", function(data) {
	for (var i in data) {
		...
		map.addMarker({
			 ...
			 /* infoWindow: {
				  content: '<p><a href="posts/' + data[i].sight_id + '">See more</a> or <button id="update-sight-btn" value="' + data[i].sight_id + '" type="submit"/>Update sight</button></p>'
			  },*/
			  id: data[i].sight_id,
			  click: function(e) {
				  infowindow.setContent('<p>' + e.title + '</p><p>' + e.description + '</p><a href="posts/' + e.id + '">See more</a> or <button onclick="updSight(' + e.id + ')" type="submit"/>Update sight</button>');
